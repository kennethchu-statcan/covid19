...
getData.Ottawa <- function(
    csv.input.hospitalization = NULL,
    jsn.input.case.and.death  = NULL,
    ...
    if ( file.exists(RData.ottawa) ) {
        ...
        DF.hospitalization <- getData.Ottawa_csv(
            input.file  = csv.input.hospitalization,
        ...
        DF.case.and.death <- getData.Ottawa_json(
            input.file  = jsn.input.case.and.death,
            output.file = "raw-ottawa-case-and-death.csv"
            )

        cat("\nstr(DF.case.and.death)\n");
        print( str(DF.case.and.death)   );
...
getData.Ottawa_json <- function(
    ...
    for ( i in seq(1,length(list.fromJSON[["features"]])) ) {
        DF.temp   <- as.data.frame( list.fromJSON[["features"]][[i]][[1]], stringsAsFactors = FALSE );
